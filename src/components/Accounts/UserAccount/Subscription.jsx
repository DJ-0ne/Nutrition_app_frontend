...
import { Loader2, Check, Phone, XCircle, AlertTriangle } from 'lucide-react';
...
const Subscription = ({ currentTier: initialTier, onUpgrade: parentOnUpgrade }) => {
  ...
  const fetchCurrentTier = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        console.log('Profile data:', data); // Debug: remove after testing

        let serverTier = data.tier || data.subscription_tier || data.subscriptionTier || initialTier;
        const expiresStr = data.subscription_expires_at || data.expires_at || data.subscriptionExpiresAt;
        ...
        if (expiresStr) {
          ...
          if (!isNaN(parsed.getTime())) {
            ...
            console.warn('Received invalid expiry date from backend:', expiresStr);
          }
        } else if (serverTier !== SubscriptionTier.FREE) {
          console.warn('Paid plan has no expiry date from backend');
        }
      ...
      try {
        ...
        if (res.ok) {
          ...
          const mapped = data.map(t => ({
            id: t.name,
            name: t.display_name || t.name.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase()),
            price: t.price === '0' || t.price === 0 || t.price === '0.00'
              ? 'Ksh 0'
              : `Ksh ${t.price}/mo`,
            features: Array.isArray(t.features) ? t.features : Object.values(t.features || {}),
  ...
  const handleDirectUpgrade = async (tierId) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setExpiresAt(data.new_tier === SubscriptionTier.FREE ? null : (data.subscription_expires_at ? new Date(data.subscription_expires_at) : null));
  ...
  const getTierCardClasses = (tier, isActive) => {
    const base = "p-6 rounded-3xl border-2 transition-all duration-500 transform hover:scale-[1.02] hover:-translate-y-1 shadow-xl group";
    if (!isActive) return `${base} border-amber-200/50 bg-white/60 hover:border-amber-200/50 hover:shadow-[0_20px_40px_rgba(0,0,0,0.04)]`;
  ...
  const getPriceColor = (tierId) => {
    switch (tierId) {
      case SubscriptionTier.PREMIUM: return "text-orange-600";
      case SubscriptionTier.PRO_LITE: return "text-amber-600";
      default: return "text-slate-600";
    }
  };

  const getActiveBadgeClasses = (tierId) => {
    const base = "text-white text-[10px] font-black px-3 py-1.5 rounded-full uppercase tracking-widest shadow-lg animate-pulse";
    switch (tierId) {
      case SubscriptionTier.PREMIUM:
  ...
  const getButtonClasses = (tierId) => {
    const base = "w-full py-4 rounded-2xl font-black text-sm tracking-widest uppercase transition-all hover:scale-[1.02] active:scale-95";
    switch (tierId) {
      case SubscriptionTier.PREMIUM:
                  ...
                  <div className="flex justify-between items-start mb-4">
                    ...
                    {isActive && (
                      <span className={getActiveBadgeClasses(tier.id)}>
                        Active
                        {tier.id !== SubscriptionTier.FREE && expiresAt && !isNaN(expiresAt.getTime())
            ...
            <div className="p-8">
              {modalMode === 'phone' && (
                <>
                  ...
                  <p className="text-slate-600 mb-8">
                    Enter your M-Pesa registered number<br />
              ...
              {modalMode === 'sending' && (
                <div className="text-center py-16">
                  ...
                  <p className="mt-6 font-medium text-slate-700">Sending M-Pesa prompt...</p>
                </div>
              )}
              ...
              {modalMode === 'waiting' && (
                <div className="text-center py-12">
                  <Loader2 className="w-12 h-12 mx-auto text-amber-600 animate-spin" />
                  <p className="mt-6 font-semibold text-lg text-slate-800">Waiting for your payment</p>
                  <p className="text-slate-600 mt-2">Check your phone and enter PIN</p>
              ...
              {modalMode === 'failed' && (
                <div className="text-center py-12">
                  ...
                  <p className="font-bold text-xl text-slate-800 mb-2">Payment Failed</p>
              ...
              {modalMode === 'success' && (
                <div className="text-center py-12">
                  ...
                  <p className="font-bold text-xl text-slate-800 mb-2">Payment Successful</p>
                  <p className="text-slate-600 mb-8">Your subscription has been activated.</p>
                  <button
                    onClick={closeModal}
          ...
          <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden">
            <div className="p-8">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h3 className="text-2xl font-black text-red-700">Warning: Downgrade to Free</h3>
              </div>

              <div className="space-y-4 text-slate-600 leading-relaxed">
                <p>
                  Your current <span className="font-semibold text-slate-800">{currentTier}</span> subscription 
                  is active until{' '}
                  {expiresAt && !isNaN(expiresAt.getTime()) ? (
                    ...
                    <span className="font-semibold text-amber-600">an unknown date</span>
                  )}.
                </p>

                <p className="font-medium text-red-600">
                  Switching to Free will immediately cancel your paid access and you will lose all premium features right away.
                </p>

                <p>
                  This action <span className="font-medium">cannot be undone</span> without making a new payment.
                </p>

                <div className="text-xs bg-amber-50 border border-amber-200 p-4 rounded-2xl">
                  Note: Your subscription would automatically revert to Free when the current paid period ends anyway.
                </div>
              </div>
