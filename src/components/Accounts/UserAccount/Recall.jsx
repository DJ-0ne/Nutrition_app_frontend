...
const IDDS_GROUPS = [
  { id: 1, name: "Cereals/Grains", examples: "Rice, maize, wheat, bread, porridge, noodles" },
  { id: 2, name: "Vitamin A-Rich Veg/Tubers", examples: "Carrots, pumpkin, sweet potatoes (orange), dark leafy greens" },
  { id: 3, name: "White Tubers/Roots", examples: "Potatoes, cassava, white yams" },
  { id: 4, name: "Other Vegetables", examples: "Tomato, onion, cucumber, cabbage" },
  { id: 5, name: "Other Fruits", examples: "Banana, apple, mango, citrus" },
  { id: 6, name: "Meat, Poultry, Offal", examples: "Beef, chicken, goat, liver, kidney" },
  { id: 7, name: "Eggs", examples: "Chicken, duck, or other eggs" },
  { id: 8, name: "Fish & Seafood", examples: "Fresh or dried fish, shellfish" },
  { id: 9, name: "Pulses, Legumes, Nuts", examples: "Beans, lentils, peanuts, soy, cashew" },
  { id: 10, name: "Milk & Milk Products", examples: "Fresh milk, yogurt, cheese" },
  { id: 11, name: "Oils & Fats", examples: "Butter, oil, ghee" },
  { id: 12, name: "Sweets/Sugar", examples: "Sugar, honey, candy, sugary drinks" },
  { id: 13, name: "Condiments/Spices", examples: "Spices, tea, coffee, sauces" },
];
...
const Recall = ({ userTier: propUserTier }) => {
  ...
  const [currentTier, setCurrentTier] = useState(propUserTier || SubscriptionTier.PREMIUM);
  ...
  const loadDataForDate = async (date) => {
    ...
    try {
      ...
      const foodsRes = await axios.get(`${apiBaseURL}/api/recall-foods/?date=${date}`, config);
      const foodsArray = Array.isArray(foodsRes.data) ? foodsRes.data : foodsRes.data.results || [];
      const foodsData = foodsArray.reduce((acc, item) => {
        const meal = item.meal_type;
        if (!acc[meal]) acc[meal] = [];
        acc[meal].push({ id: item.id.toString(), food: item.food_name, finished: item.finished });
        return acc;
      }, {});

      setRecallData((prev) => ({ ...prev, [date]: foodsData }));

      const iddsRes = await axios.get(`${apiBaseURL}/api/idds-responses/?date=${date}`, config);
      const iddsArray = Array.isArray(iddsRes.data) ? iddsRes.data : iddsRes.data.results || [];
  ...
  const handleAddItem = (mealValue, foodName) => {
    ...
    const isDuplicate = currentMealItems.some(item => item.food.toLowerCase() === foodToAdd.toLowerCase());

    if (isDuplicate) {
      toast.error("Meal already logged", { description: "This food is already added to this meal today." });
      return;
    }

    const newItem = { id: Date.now().toString() + Math.random(), food: foodToAdd, finished: true };
  ...
  const removeItem = (mealValue, itemId) => {
    setRecallData((prev) => ({
      ...
      [selectedDate]: {
        ...(prev[selectedDate] || {}),
        [mealValue]: (prev[selectedDate]?.[mealValue] || []).filter((item) => item.id !== itemId),
      },
    }));
  };
    ...
    try {
      ...
      Object.keys(currentDayData).forEach((meal) => {
        currentDayData[meal].forEach((item) => {
          foods.push({ meal_type: meal, food_name: item.food, finished: item.finished });
        });
      });
      ...
      await axios.post(`${apiBaseURL}/api/save-recall/`, { date: selectedDate, foods, idds }, config);
      ...
      if (errorMsg.includes("duplicate key") || errorMsg.includes("already exists")) {
        toast.error("Meal already logged", { description: "This food is already logged for this date and meal type." });
      } else {
  ...
  const generateIDDSPDF = () => {
    ...
    const doc = new jsPDF();
    doc.setFontSize(20);
    doc.text("Individual Dietary Diversity Score (IDDS) Report", 14, 22);
    doc.setFontSize(12);
    doc.text(`Date: ${selectedDate}`, 14, 32);

    const score = Object.values(iddsScores[selectedDate] || {}).filter(Boolean).length;
    doc.text(`Total Score: ${score}/13`, 14, 40);

    const tableData = IDDS_GROUPS.map((g) => [g.name, iddsScores[selectedDate]?.[g.id] ? "YES" : "NO"]);

    doc.autoTable({ startY: 50, head: [["Food Group", "Consumed"]], body: tableData });

    doc.save(`IDDS_Report_${selectedDate}.pdf`);
  };
  ...
  if (loading && Object.keys(currentDayData).length === 0) {
    return (
      <div className="flex min-h-screen bg-gradient-to-br from-amber-50 via-white to-amber-50/30 items-center justify-center">
        <div className="text-amber-600 font-medium">Loading your recall data...</div>
      </div>
    );
  }
  ...
  return (
    <div className="flex max-h-screen overflow-hidden bg-gradient-to-br from-amber-50 via-white to-amber-50/30">
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="w-fit mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20 p-4 lg:p-8">
          <button
          ...
          <header className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-3 lg:gap-4 mb-4 lg:mb-6">
            <div className="w-full lg:w-auto">
              ...
              <p className="text-xs lg:text-sm text-slate-500 font-medium mt-1">
                Reconstruct your nutritional intake. Please specify if you finished each portion.
              </p>
            </div>
            <div className="flex flex-col items-start lg:items-end gap-2 w-full lg:w-auto">
              <label htmlFor="date-select" className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">
                Select Date
              </label>
              <input
                id="date-select"
          ...
          <div className="bg-gradient-to-r from-amber-500 to-orange-600 rounded-3xl p-6 text-white shadow-xl shadow-amber-200 relative overflow-hidden">
            <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
              <div>
                <h3 className="text-xl font-black tracking-tight mb-1">AI Food Analysis</h3>
                <p className="text-amber-100 text-sm font-medium">Upload a photo of your meal for instant logging.</p>
                <input type="file" accept="image/*" className="hidden" id="food-photo-upload" onChange={handlePhotoScan} />
                <label
            ...
            <div className="bg-white border-2 border-amber-100 rounded-2xl p-6 shadow-lg animate-in fade-in zoom-in-95 duration-300">
              <div className="flex justify-between items-start mb-4">
                <div>
                  ...
                  <p className="text-xs text-slate-500">Add to the correct meal</p>
                </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  <label className="text-sm font-medium text-slate-700">Add to meal:</label>
                  <select value={selectedMealForAI} onChange={(e) => setSelectedMealForAI(e.target.value)} className="bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm">
                    {MEAL_TYPES.map((m) => (
                ...
                <div className="space-y-2">
                  {aiSuggestions.map((food, idx) => (
                    <div key={idx} className="flex items-center justify-between bg-amber-50/50 p-3 rounded-xl border border-amber-50">
                      <span className="text-sm font-medium text-amber-900">{food}</span>
                      <button
          ...
          <div className="bg-white/70 backdrop-blur-xl p-8 rounded-3xl shadow-[0_20px_40px_rgba(0,0,0,0.04)] border border-white/50 space-y-8">
            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest text-center">
              Timeline for {new Date(selectedDate).toLocaleDateString(undefined, { weekday: "long", year: "numeric", month: "long", day: "numeric" })}
            </p>
              ...
              <div key={meal.value} className="space-y-4 group">
                ...
                <div className="space-y-2">
                  {(currentDayData[meal.value] || []).map((item) => (
                    <div key={item.id} className="flex items-center justify-between p-3 bg-white/60 rounded-xl border border-amber-50 shadow-sm transition-all hover:shadow-md hover:scale-[1.01]">
                      <span className="font-bold text-slate-700 text-sm">{item.food}</span>
                      <div className="flex items-center gap-3">
                        <button
                          onClick={() => toggleFinished(meal.value, item.id)}
                          className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider border transition-all duration-300 ${
                            item.finished
                              ? "bg-emerald-100 text-emerald-700 border-emerald-200 hover:bg-emerald-200"
                              : "bg-amber-100 text-amber-700 border-amber-200 hover:bg-amber-200"
                          }`}
                        >
                          {item.finished ? "Finished: Yes" : "Finished: No"}
                        </button>
                        <button
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    onChange={(e) => setCurrentInput({ ...currentInput, [meal.value]: e.target.value })}
                    onKeyDown={(e) => e.key === "Enter" && handleAddItem(meal.value)}
                  />
                  <button
            ...
            <div className="pt-8 border-t border-amber-100">
              <header className="mb-6 flex flex-col md:flex-row justify-between items-end gap-4">
                <div>
                  <h3 className="text-xl font-black text-slate-800 tracking-tight">Individual Dietary Diversity Score (IDDS)</h3>
                  <p className="text-sm text-slate-500 font-medium mt-1">
                    Did you consume any of the following food groups yesterday?
                    <span className="block text-xs text-amber-400 mt-1 font-bold uppercase tracking-wide">Strictly based on the last 24 hours</span>
                  </p>
                </div>

                {currentTier !== SubscriptionTier.FREE ? (
                  <button
                    onClick={generateIDDSPDF}
              ...
              <div className="bg-white/60 rounded-2xl border border-amber-50 overflow-hidden overflow-x-auto">
                <table className="w-full min-w-[600px]">
                  <thead className="bg-amber-50/50 border-b border-amber-50">
                    <tr>
                      <th className="text-left py-4 px-6 text-[10px] font-black text-slate-400 uppercase tracking-widest w-16">#</th>
                      <th className="text-left py-4 px-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Food Group & Examples</th>
                      <th className="text-center py-4 px-6 text-[10px] font-black text-slate-400 uppercase tracking-widest w-32">Consumed?</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-50">
                    {IDDS_GROUPS.map((group) => {
                      const isChecked = iddsScores[selectedDate]?.[group.id] || false;
                      return (
                        <tr key={group.id} className="hover:bg-white/80 transition-colors group">
                          <td className="py-4 px-6 text-sm font-bold text-slate-400">{group.id}</td>
                          <td className="py-4 px-6">
                            <p className="font-bold text-slate-700">{group.name}</p>
                            <p className="text-xs text-slate-400 mt-0.5 font-medium">{group.examples}</p>
                          </td>
                  ...
                  <tfoot className="bg-slate-50/80 border-t border-amber-50">
                    <tr>
                      <td colSpan={2} className="py-4 px-6 text-right text-xs font-black text-slate-500 uppercase tracking-widest">
                        Total Score (0-13)
                      </td>
                      <td className="py-4 px-6 text-center">
                        <div className="inline-flex items-center justify-center w-12 h-12 rounded-full bg-amber-600 text-white font-black text-lg shadow-lg shadow-amber-300/50">
                          {Object.values(iddsScores[selectedDate] || {}).filter(Boolean).length}
                        </div>
                      </td>
                    </tr>
                  </tfoot>
