...
export default function AdminLayout({ children, user, logout }) {
  ...
  return (
    <div className="flex min-h-screen bg-slate-50 text-slate-900 font-sans">
              ...
              >
                {({ isActive }) => (
                  <>
                    ...
                    {/* ← NEW: Notification badge / dot */}
                    {item.showBadge && unreadCount > 0 && (
                      <div className="ml-auto flex-shrink-0 bg-red-500 text-white text-[10px] font-bold rounded-full w-5 h-5 flex items-center justify-center shadow-md ring-2 ring-white">
      ...
      {/* Mobile Sidebar */}
      <aside
        className={`lg:hidden fixed top-0 left-0 h-screen w-64 bg-white/80 backdrop-blur-xl border-r border-slate-200 shadow-2xl z-50 transform transition-transform duration-300 ease-in-out ${
            ...
            return (
              <NavLink
                ...
                onClick={(e) => {
                  ...
                  closeMobileSidebar(); // Close sidebar on link click
                }}
                className={({ isActive }) =>
                  `w-full flex items-center gap-4 px-4 py-3.5 rounded-2xl transition-all duration-500 group relative overflow-hidden ${
                    isActive
      ...
      <div className="flex-1 flex flex-col">
        <header className="bg-white/80 backdrop-blur-xl border-b border-slate-200 sticky top-0 z-50 px-4 lg:px-10 py-4 flex justify-between items-center shadow-sm transition-all duration-300">
          <div className="flex items-center gap-4">
            ...
            <button
              className="lg:hidden text-slate-600 hover:text-indigo-600 transition-colors duration-300"
            ...
            <h1 className="text-2xl font-black text-transparent bg-gradient-to-r from-indigo-600 to-violet-600 bg-clip-text">
              Admin Panel
            </h1>
          </div>
